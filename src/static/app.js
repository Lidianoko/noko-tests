      ...
      Object.entries(activities).forEach(([name, details]) => {
        ...
        const participantsMarkup = participants.length > 0
          ? participants.map((email) => `
              <li class="participant-row" data-activity-name="${name}" data-email="${email}">
                <span class="participant-email">${email}</span>
                <button class="participant-delete-btn" type="button" aria-label="Remove ${email}" title="Unregister ${email}">
                  ✕
                </button>
              </li>
            `).join("")
